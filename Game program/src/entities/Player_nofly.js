class Player extends GameObject {
  ...
  update(mapManager, entities, camY = 0) {
    //--生存狀態更新--
    // A. 低於地面即死，擺盪期間不算
    ...
    // D. 處理輸入與物理
    this.handleInput(mapManager, camY);
    // --- 扇形擺盪 ---
    if (this.isHooked) {
      ...
      this.pos.x =
        this.hookPoint.x +
        this.tongueLen * Math.cos(currentAngle) -
        this.width / 2;
      this.pos.y =
        this.hookPoint.y +
        this.tongueLen * Math.sin(currentAngle) -
        this.height / 2;
  ...
  //輸入處理---
  handleInput(mapManager, camY = 0) {
    // 左鍵擺盪
    if (mouseIsPressed && mouseButton === LEFT) {
      if (!this.isHooked) this.shootTongue(mapManager, camY);
  ...
  shootTongue(mapManager, camY = 0) {
    ...
    for (let wall of walls) {
      ...
      if (wall.y < floorY) {
        ...
        if (
          worldMouseX >= wall.x &&
          worldMouseX <= wall.x + wall.w &&
          worldMouseY >= wall.y &&
          worldMouseY <= wall.y + wall.h
        ) {
          ...
          if (d < this.maxGrabDist) {
            ...
            this.startAngle = atan2(
              pCenterY - this.hookPoint.y,
              pCenterX - this.hookPoint.x,
            );
  ...
  display() {
    ...
    if (this.isAttacking) {
      ...
      let atkEndX =
        this.facing === -1
          ? this.pos.x + this.width + this.attackRange
          : this.pos.x - this.attackRange;
      line(
